...
const StarCatcherGame = () => {
    ...
    // --- Refs for direct access without re-rendering ---
    const canvasRef = useRef(null);
    const gameContainerRef = useRef(null);
    // Refs to hold game variables that change frequently but shouldn't trigger re-renders
    ...
    const drawPlayer = useCallback((ctx) => {
        ...
        ctx.fillStyle = 'rgb(165, 180, 252)'; // Indigo-300
        ctx.strokeStyle = 'rgb(199, 210, 254)'; // Indigo-200
        ctx.lineWidth = 3;
        ctx.beginPath();
    ...
    const gameLoop = useCallback(() => {
        ...
        for (let i = starsRef.current.length - 1; i >= 0; i--) {
            ...
            if (star.x < playerRef.current.x + playerRef.current.width &&
    ...
    useEffect(() => {
        ...
        const resizeCanvas = () => {
            ...
            canvas.width = containerWidth - (window.innerWidth < 640 ? 32 : 48); // Adjust for padding
            canvas.height = canvas.width * (9 / 16);
            
            // Recalculate player position on resize when game is running
    ...
    // Effect to start the game loop when `isGameOver` becomes false
    useEffect(() => {
        if (!isGameOver) {
           gameLoopIdRef.current = requestAnimationFrame(gameLoop);
        } else {
    ...
    return (
        <div className="bg-slate-900 text-white flex items-center justify-center min-h-screen font-sans p-4">
...
// Ganti nama font default di `index.html` jika perlu.
// Body font style diatur di `index.css` via @tailwind base
function App() {
